...
- Security: A git global option before the subcommand (`git -C . reset --hard`, `git -c x=y clean -fdx`, `git --git-dir=.git branch -D x`) shifted the subcommand off the position every `GIT-*` rule expected, so the command scored Safe. The scanner now resolves where the subcommand actually starts, including a chain of several options or one it does not recognize, and re-runs the `GIT-*` rules and regex patterns against it. A chain that leaves more than 16 possible subcommand positions is not scanned and gets a Warn of its own (`SCAN-004`). (ADR-041) ([GHSA-7564-rmfj-ch4p](https://github.com/IliasAlmerekov/aegis-shellguard/security/advisories/GHSA-7564-rmfj-ch4p))
- Fixed: A quoted heredoc read by `cat`, `tee` or `jq` is no longer scanned as a command when no shell can reach its output, so `body=$(jq -c . <<'JSON' ...)`, `git commit -m "$(cat <<'EOF' ...)"`, `true; cat > f <<'EOF'` and a plain `cat <<'EOF'` stop being denied because their text mentions a dangerous command. A pipe, a process substitution, a subshell, a `{ ...; }` group, a loop or other compound command, a comment, an earlier `exec`, a line continuation, a write to a descriptor above 2, or an enclosing `$(...)` other than an assignment or a `git`/`gh` message value keeps the body scanned. For an assignment's `$(...)`, the body stays scanned unless every later reference to the captured variable is provably forwarding only: a data flag value of a top-level `git` (`-m` only under `commit`/`tag`/`merge`/`notes`, no global option first) or `gh` (subcommand read from the two words right after `gh`, message, title or body flag only under `gh pr create`/`edit`/`comment`/`review`/`merge` or `gh issue create`/`edit`/`comment`, or `-f` only under `gh api`), `curl` (`--data-raw`, or `-d` and similar when the body does not start with `@`) or `jq` (`--arg`/`--argjson`) invocation, or an argument of `echo`, or of `printf` when its first argument does not start with `-` and the reference sits after that format string, with no pipe alongside it, no disallowed redirect and no laundering through another variable. A wrapper (`sudo $x`, `env $x`, `nohup $x`, `find -exec $x \;`), a grouping construct (`($x)`, `{ $x; }`, an `if`/`for`/`while` body), a parameter expansion (`${x:-}`, `${x%%foo}`), a second capture (`z=$($x)`), a nameref (`declare -n r=x; $r`), a here-string (`bash <<< "$x"`), an `alias`, `trap` or `PROMPT_COMMAND` store, a write then run (`echo "$x" > f.sh; sh f.sh`), or any program or flag outside that list, a `gh` subcommand outside that list (`gh pr checkout 1 -b "$x"`), or a `printf` with a leading option (`printf -v y "$x"`, which sends the value to a second variable), keeps the body scanned too. (ADR-042) (#396, #432)
...
- Security: Language-aware analysis now reaches every command in a compound command, not just the first. Before this fix, a script launched after `;`, `&&`, `||`, `&`, a pipe, or a newline, or behind a heredoc, subshell, brace group, command or process substitution, `case` arm, shell keyword, function definition, named `coproc`, leading or stdin redirection, `xargs`, `env -S`/`--split-string`/`-C`, `nodejs`, or an unenumerated launcher word (`setsid`, `strace`, …), skipped analysis and was auto-approved. Only a literal `cd -- <path>` tracks the working directory through chained commands and wrapper bodies; `pushd`, `popd`, `source`, and `.` degrade it for later commands instead, and analysis degrades wherever that directory is unknown. Wrappers nested deeper than 8 levels degrade instead of recursing. A dynamic or aliased program word with its own operand, every distinct path-like operand of an unclaimed stage (not only the first), and an interpreter smuggled through a git/man/less executor option or environment variable now degrade or route the same way. (#384, #430) ([GHSA-c5pv-6qf7-w4cc](https://github.com/IliasAlmerekov/aegis-shellguard/security/advisories/GHSA-c5pv-6qf7-w4cc))
...
- Fixed: The 800-line file budget test skipped every file when the checkout path had a `.claude` or `target` component, so an oversized `builtins_a.rs` passed it. It now checks only directory names inside the workspace.
- Changed: The CI performance job runs the new router benchmarks and gates four `PreparedPlanner::plan()` rows, so a routing slowdown fails the build. (#437)
...
- Removed: The nine `src/` re-export shims over `audit`, `config`, `decision`, `snapshot`, `ui::confirm`, and `interceptor`'s scanner/patterns/parser trees are deleted. Every import now names the crate that defines the type directly, instead of routing through a binary-crate module that just re-exported it. The `aegis` binary's public module list (`src/lib.rs`, pinned by `tests/architecture_boundaries.rs`) keeps only the modules external consumers use. (#281)
- Changed: The built-in scanner static, `assess()`, and `scanner_for()` move from the binary's `interceptor` module into `aegis-scanner`; `src/interceptor` is now a re-export shim. The mutex-guarded cache of custom-pattern scanners is removed rather than moved — a `RuntimeContext` is built once per process, so it never held a second entry. Config validation no longer builds a custom-pattern scanner it discards before `RuntimeContext` builds its own on the direct in-memory config path, removing that double `Scanner::try_new` call (#319). The layered file-load path (`AegisConfig::load`) no longer rebuilds a scanner per config layer either — a config with custom patterns in both the global and project layers used to pay `Scanner::try_new` up to three times per invocation; now `RuntimeContext` builds it exactly once, and a config-layer path is still attributed to the first invalid custom pattern if that single build fails (#399). (#280)
...
- Fixed: A nowdoc (`<<'EOF'`, `<<"EOF"`, `<<\EOF`) disables bash expansion in its body, backticks and `$(...)` included, but the scanner pulled that inert text out as an independent scan target anyway — a markdown code span quoting a dangerous-looking command inside a `gh pr create --body "$(cat <<'EOF' ... EOF)"` call could trip a pattern and get denied in non-interactive mode. Backtick/`$(` markers inside a nowdoc body are now masked before segmentation, unless the heredoc's target is itself a shell or scripting interpreter, which still executes that text verbatim. (#344, #368, #369, #370)
- Fixed: `rtk proxy <cmd>` resolved its effective program to the literal token `proxy` instead of `<cmd>`'s own program, since launcher-prefix stripping always skipped exactly one token. No rule keys on `proxy`, so any command run through `rtk proxy` was auto-approved `Safe` with no Snapshot — a bare `git rebase` included, which Aegis denies outright when run directly. (#339, #368, #370)
- Fixed: A directly executed file (`/tmp/actionlint file.yml`) is a shebang-script candidate — routing reads it looking for a `#!` line before treating it as analyzable source. A read that exceeded the script-file budget or decoded as non-UTF-8 fell through to the same catch-all as a genuine analysis failure, degrading to `AnalysisStatus::Degraded` and forcing an unconditional non-interactive deny (ADR-022 §5) — so any compiled binary invoked by path was denied purely for being a binary. Both read errors are in fact conclusive (a shebang line is a short ASCII prefix neither can carry) and now resolve to `NotApplicable`, the same outcome a successful read with no `#!` match already gets. ADR-022 §6 is amended to explain why the narrowing applies only to the direct-exec shebang check. (#345, #369, #370)
- Fixed: An audit write failure (e.g. a read-only `~/.aegis` on a managed workspace) printed a bare `io error: Read-only file system (os error 30)` with no path and no way to recover. Audit writes stay a hard failure by design, but the error now names the configured audit path and the existing `AEGIS_AUDIT_PATH` override so the operator can redirect the log and unblock every command in one step. (#367)
- Changed: `[workspace.package]` and `[workspace.dependencies]` in the root `Cargo.toml` hold the version, edition, and internal crate paths once; every crate and dependent now inherits them with `.workspace = true` instead of repeating the literal on each of the twelve crates. A release version bump is one edit instead of one per crate.
- Changed: `.github/dependabot.yml` groups every Dependabot security-update PR by ecosystem instead of opening one per advisory. (#366)
...
- Fixed: The cargo cache key includes the toolchain version, so a Rust bump no longer restores a `target/` built by the previous compiler and keeps the stale objects in the cache entry.
- Security: Every checkout in `ci.yml` and `release.yml` sets `persist-credentials: false`, so no job leaves the workflow token in `.git/config` where a build script or a dependency could read it. This matters most on the release jobs, which run with `contents: write`. `release.yml` also gained a `concurrency` group and per-job timeouts, and both readers of `.github/build-targets.json` now fail on an empty target list instead of building nothing and publishing a Release with no binaries.
- Security: Required recovery is `Ready` only when every applicable Snapshot plugin created its Snapshot. A pass where git succeeds and a configured database provider fails now stops at the Recovery prompt and records `partial_snapshot_coverage`, instead of reading as full coverage and running unprompted. Total failure still records `no_snapshot_available` and still fails closed. The prompt names which of the two happened. (#312, ADR-036)
