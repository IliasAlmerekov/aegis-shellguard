...
- Security: Language-aware routing now examines every top-level list segment and every pipeline stage of a compound command, not just the first — `;`, `&&`, `||`, a backgrounded `&`, a pipe, and a newline each get their own route. This closes the gap for a script named behind a heredoc-prefixed command (`cat <<EOF && python3 evil.py`) or one chained straight onto a heredoc's own marker line, a subshell or brace group, command or process substitution, a `case` arm, a shell keyword (`if`/`while`/`until`/`do`/...), a function definition, a named `coproc`, and the `xargs`/`env -S`/`nodejs` launchers — all previously skipped language analysis and auto-approved. A leading redirection (`>out python3 evil.py`) or a stdin redirection (`python3 < evil.py`) no longer hides the program behind it either. `cd`/`pushd`/`popd`/`source`/`.` cwd tracking now follows across chained segments and into a wrapper's own body, degrading on any construct it cannot resolve, and wrapper-peeling recursion is bounded so a deeply nested `((((...))))` cannot exhaust the stack or cost work quadratic in nesting depth. Separately, a `case`/`esac` word only suspends list splitting when it is a real keyword in command position, not a plain argument — `grep case f; python3 evil.py; grep esac f` used to auto-approve the smuggled `python3` call. (#384, #430)
