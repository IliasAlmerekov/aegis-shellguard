...
- Changed: The built-in scanner static, `assess()`, and `scanner_for()` move from the binary's `interceptor` module into `aegis-scanner`; `src/interceptor` is now a re-export shim. The mutex-guarded cache of custom-pattern scanners is removed rather than moved — a `RuntimeContext` is built once per process, so it never held a second entry. Config validation no longer builds a custom-pattern scanner it discards before `RuntimeContext` builds its own, removing the last double `Scanner::try_new` call on that path (#319). (#280)
