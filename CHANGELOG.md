...
- Security: `ci.yml` checks out with `persist-credentials: false`, so no job leaves the workflow token in `.git/config`. `release.yml` gained a `concurrency` group and per-job timeouts.
