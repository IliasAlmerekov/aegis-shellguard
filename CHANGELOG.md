...
- Security: Language-aware analysis now reaches every command in a compound command, not just the first. Before this fix, a script launched after `;`, `&&`, `||`, `&`, a pipe, or a newline, or behind a heredoc, subshell, brace group, command or process substitution, `case` arm, shell keyword, function definition, named `coproc`, leading or stdin redirection, `xargs`, `env -S`, or `nodejs`, skipped analysis and was auto-approved. `cd`, `pushd`, `popd`, `source`, and `.` now carry the tracked working directory through chained commands and wrapper bodies, and analysis degrades wherever that directory is unknown. Wrappers nested deeper than 8 levels degrade instead of recursing. (#384, #430)
