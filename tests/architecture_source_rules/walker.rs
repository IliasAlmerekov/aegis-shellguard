//! Production-code walker for the architecture source rules: strips
//! `#[cfg(test)]` items and resolves which files a parent module declares
//! under `#[cfg(test)]`.
...
pub fn strip_test_code(content: &str) -> String {
...
fn collect_rs_files(dir: &Path, out: &mut Vec<PathBuf>) {
    for entry in fs::read_dir(dir).unwrap_or_else(|e| panic!("read_dir {dir:?}: {e}")) {
        ...
        if path.is_dir() {
            ...
            let is_cargo_target_dir = matches!(name, "tests" | "benches" | "examples")
                && dir.join("Cargo.toml").is_file();
            if !is_cargo_target_dir {
...
pub fn production_rs_files(relative: &str) -> Vec<(String, String)> {
...
pub fn contains_ident(source: &str, ident: &str) -> bool {
...
mod resolver_self_tests {
    ...
    #[test]
    fn the_walker_skips_crate_level_test_bench_and_example_targets() {
        let crates = paths("crates");
        assert!(crates.iter().any(|p| p == "crates/aegis-config/src/lib.rs"));
        for target in ["tests", "benches", "examples"] {
            ...
            assert!(
                ...
                "walker returned a crate-level {target} file"
            );
        }
    }
