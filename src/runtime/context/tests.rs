...
/// A config loaded via `AegisConfig::load_for` against an explicit
/// (non-process) `current_dir`/`home_dir` must attribute a scanner failure
/// to *that* project config file, not to whatever the process's real `cwd`/
/// `HOME` happen to be — those roots have nothing to do with how this config
/// was assembled (issue #400).
#[test]
