...
(`case`, `do`, `if`, `while` and the like) before the marker, and the heredoc
sits at top level, in `NAME=$(...)`, or in the `$(...)` value of a `git`/`gh`
message flag (`-m`, `--message`, `-t`, `--title`, `-b`, `--body`) with no
subshell, process-substitution `(` or `{ ...; }` group open around it. Any
other shape keeps the body scanned (ADR-040).
